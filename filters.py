...
def wierd_grad(bgr_frame):
    ...
    mapped_grad = (grad * (255. / grad.max())).astype(np.uint8)
    return mapped_grad

a = np.array([[-.3, -.1, -.1], [.2, .7, .1], [.2, .2, .6]]).T
...
def filter1(bgr_frame):
    ...
    b2 = np.where(.5*b > np.abs(g - r), b, np.maximum(g, r))
    g2 = np.where(.5*g > np.abs(b - r), g, np.maximum(b, r))
    r2 = np.where(.5*r > np.abs(b - g), r, np.maximum(b, g))
